...
# convert the image to grayscale
image_greyscale = convert_to_grayscale(image)

image = normalize_image(np.min(image_greyscale), np.max(image), 0, 255, image_greyscale)
...
for i in range(m):
    for j in range(n):
        image[i, j] = hist_eq[image_hef[i, j]]

imageio.imwrite('/home/arif/PycharmProjects/X-ray-Image-Enhancement/data/010_hef.jpg', image)
